...
- **The next task** is the first lesson in the curriculum README's status table that is not "Built" (currently M3).
- **Unit 3B Faults** (added 2026-09-26) holds B7–B9 and new lessons B10–B18. It shares the B prefix, and its teaching order is not its ID order; B17 and B18 are taught after Unit 6. The catalog lists it that way, and its lesson files are in `src/lessons/unit-3b-faults/`. B6, B7, B8, B9, and B11 were built early at the instructor's request; treat them as built, and revisit them when their prerequisites are built (see their "As built" notes). B6 built the first stereonet (`Stereonet.js`) and a 3D Mohr plot (`FrictionMohrPlot.js`); O3, S9, B3, and B4 extend those. B8 built the fault lab (`FaultScene.js`, with shared Earth-block helpers in `earthBlock.js`), the stereonet's kinematic layers, and O4's rake functions in `orientation.js`; O4 extends those. B9 built the fault-growth lab (`FaultGrowthScene.js`, models in `faultGrowth.js`) and a general x–y plot (`XYPlot.js`); F6 extends those. B11 built the fault-zone lab (`FaultZoneScene.js`, models in `faultRocks.js`), Sibson's chart (`FaultRockChart.js`), and the damage map, depth column, and architecture gauge (`FaultZonePanels.js`); B12, B13, B16, and R6 extend those. Instructor photographs are not integrated yet: discuss with the instructor first.
...
- **No engineering statics:** no moments/torques, free-body rotation, support reactions, section cuts, internal resultants, bending, or torsion. They were removed on purpose. The one exception is a force balance along a frictional sliding surface (B13's spring-slider, B14's thrust sheets).
