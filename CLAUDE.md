...
- **The next task** is the first lesson in the curriculum README's status table that is not "Built" (currently M3). B6 and B7 were built early at the instructor's request; treat them as built, and revisit them when their prerequisites are built (see their "As built" notes). B6 built the first stereonet (`Stereonet.js`) and a 3D Mohr plot (`FrictionMohrPlot.js`); O3, S9, B3, and B4 extend those.
