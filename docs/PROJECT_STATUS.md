...
The current release (`0.10.0`) contains four fully built lessons: **M1 Vectors and components** and **M2 Trigonometry of projection**, which run in the vector laboratory, and **B6 Friction and reactivation of existing planes** and **B7 Anderson's theory of faulting**, both built early for classroom use on an NED Earth block. B7 added the first Mohr diagram; B6 added the 3D Mohr diagram and the first stereonet. Build 00 (0.6.0) removed the engineering-statics sequence and introduced the lesson registry and the equation–model binding panel. S2, S3, S7, and S10 are still early seed versions. The remaining 40 lessons are planned, and each has a build spec.
...
- The equation binding covers the vector, Anderson, friction, and force laboratories; `StressScene` does not yet support symbol highlighting.
