...
The current release (`0.12.0`) contains six fully built lessons: **M1 Vectors and components** and **M2 Trigonometry of projection**, which run in the vector laboratory, and **B6 Friction and reactivation of existing planes**, **B7 Anderson's theory of faulting**, **B8 Fault geometry, slip, and kinematic axes**, and **B9 Fault anatomy and growth**, all built early for classroom use on an NED Earth block. B7 added the first Mohr diagram; B6 added the 3D Mohr diagram and the first stereonet; B8 added the fault laboratory, a well log, and the stereonet's kinematic layers (slip, P/T/B, beach balls); B9 added the fault-growth laboratory and a general x–y plot. Build 00 (0.6.0) removed the engineering-statics sequence and introduced the lesson registry and the equation–model binding panel. S2, S3, S7, and S10 are still early seed versions. The remaining 38 lessons are planned, and each has a build spec.
...
- [x] B9 (built early, 0.12.0): fault anatomy and growth in eight steps: the fault core, slip surface, damage zone, and fault rocks in an outcrop; displacement dying out at the tip line, seen in a section that cuts the block; the fault surface as a displacement map with contours and profile lines (elliptical and linear models); D = c Lⁿ on log–log axes; relay ramps between overlapping segments; breaching and the displacement deficit of a linked fault; normal and reverse drag; and segmented faults on maps. Its displacement models are stated idealizations, and its displacements are large (D/L = 0.1) so they read on screen.
...
- The equation binding covers the vector, Anderson, friction, fault, fault-growth, and force laboratories; `StressScene` does not yet support symbol highlighting.
...
- B9 was built after B8 but ahead of B1–B5. Its displacement fields are stated idealizations (elliptical or linear tip-line profiles, bell-shaped relay segments, an exponential fade away from the fault, and added fields for several faults), not a mechanical model, and its displacements are large (D/L = 0.1) so they read on screen. The fault-rock drawings and the damage-zone fractures are schematic, and the D–L data are synthetic.
