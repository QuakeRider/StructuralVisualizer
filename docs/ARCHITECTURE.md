...
`src/visualization/FaultGrowthScene.js` is the fault-growth laboratory (B9), in the same NED Earth block at a chosen scale (1000 m, or 40 m for the outcrop). Marker horizons are grids of points moved by `faultDisplacement` for every fault, with triangles that span a fault dropped so each bed ends at its cutoffs; beds can be shaded by depth with structure contours (relay). Fault surfaces (ellipses inside their tip lines, or rectangles) are colored by displacement with contour loops, and the fault core and damage zones are slabs of the block with instanced fractures. Screen-width lines use Three.js `LineSegments2`. A section can cut the block (clipping planes remove the rock south of it), and the camera has 3D, map, section, and fault-face presets, with a zoom for the relay. It sits in a `.split-viewport` beside `XYPlot.js`, a general x–y plot (lines, points, markers, bands, and log axes, with a displacement color bar), or `FaultRockPanel.js`, chosen by `data-panel`. `plotKit.js` holds the displacement color scale shared by both.
